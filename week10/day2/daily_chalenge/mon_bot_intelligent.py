...
def resumer_avec_llm_simule(sujet_du_briefing, liste_articles, ctx: ContexteSimule):
    ctx.info(f"Début du résumé sur '{sujet_du_briefing}'")
    time.sleep(1)
...
def enregistrer_fichier(nom_fichier, contenu_briefing):
    dossier_sortie = "briefings_magueule"
...
if __name__ == "__main__":
    ...
    if not sujet_utilisateur:
        sujet_utilisateur = "hydratation"  # Sujet par défaut lié à tes articles simulés
    ...
    # ⚠️ sécurité selon le type de retour
    articles = result_recherche if isinstance(result_recherche, list) else result_recherche.get('resultats', [])
